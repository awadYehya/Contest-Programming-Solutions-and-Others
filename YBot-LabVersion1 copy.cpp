...
// but this bot loves to save 4's for later since it
// is useful when you knock back any enemy with 1 tile
// or you are knocked back with 1 tile
...
int getMaxNotModeTile(vector <int> tiles) {
	...
	for (int i = 0; i < tiles.size(); ++i)
	{
		if (tiles[i] > MaxTile && tiles[i] != Mode && tiles[i] != 4) MaxTile = tiles[i] ;
	}
...
string getSmartDefend(int playerNum, int myPos, int hisPos, vector<int> tiles) 
{
	...
	else 
	{
		for (int countK = 0; countK < tiles.size(); ++countK)
		{
			...
			// New in YBot-Improve
			if (canDefendWithTile(tiles,tiles[countK],targetTile) && (targetTile <= myPos || moveOrRetreat == "move")) {
				// if the target tile exists then we use it.
	...
	return moveAnywhereSafe(playerNum, myPos, hisPos, tiles) ; // this will make sure it does not retreat off the island.

	// old code.
	cerr << "Last option used! " << endl ;
...
string moveAnywhereSafe(int playerNum, int myPos, int hisPos, vector<int> tiles)
{
	...
	for (int countK = 0; countK < tiles.size(); ++countK)
	{
		for (int someTile = 3; someTile > 0; --someTile)
		{
	...
	// retreats same as YBot-Improved-No2
	for (int countK = 0; countK < tiles.size(); ++countK)
	{
		for (int someTile = 1; someTile < 4; ++someTile)
		{
